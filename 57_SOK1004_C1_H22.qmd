...
# gir navnet dataen navnet bnp_data1

bnp_data1 = as.tibble(df[[2]])

bnp_data1 = rename(bnp_data1 , "verdi" = value, "tid" = Tid, var = "ContentsCode" )

head(bnp_data1)

sapply(bnp_data1, class)

bnp_data1$tid = as.integer(bnp_data1$tid)
...
lopende <- subset(bnp_data1, var == "BNP" & tid >= 2000)
fast <- subset(bnp_data1, var == "MEMOBNP" & tid >= 2000)

ggplot() +
geom_line(data = fast, aes(x = tid, y = verdi, col = var)) +
geom_line(data= lopende, aes(x = tid, y = verdi, col = var)) +
scale_y_continuous(labels = scales :: comma) + 
labs(title = "BNP i løpende og faste priser") + theme_bw()
...
# Vi ser at den faste bnp prisen ikke utvikler seg like raskt som den løpende. Dette er fordi den tar bort effekten av pris endring over tid.
# I en internasjonal sammenligning vil den faste bnp prisen (reel), være den mest aktuelle.
# I en internasjonal sammenligning vil vi se¨på bnp med faste priser som utgangspunkt, på den måten vil man kunne sammenligne utviklingen på tvers av nasjoner. 
...
# Koden gjør tabbelen med data kortere. Den flyttet verdiene fra variablen BNP og MEMOBNP. Vi kan bruke pivot_wide() funksjonen når data er spredt utover flere rader. Når vi bruker funsjonen blir dataen mer ryddig og oversiktlig - enklerer å håndtere. 
...
# Lager egen kode for endring med funksjonen `lag()`
...
bnp_wide <- bnp_wide %>% 
  relocate("LBNP", .before = "BNP")

bnp_wide <- bnp_wide %>% 
  relocate("LMEMOBNP", .before = "MEMOBNP" )
...
bnp_long <- bnp_wide %>% 
pivot_longer(2:9, names_to = "BNP_Type", values_to = "verdi")
```
...
nominel <- subset(bnp_long, BNP_Type == "ProsBNP" & tid)

reel <- subset(bnp_long, BNP_Type == "ProsMEMOBNP" & tid)

ggplot() +
geom_smooth(data= nominel, aes(x= tid, y = verdi, col = BNP_Type)) + 
geom_smooth(data= reel, aes(x = tid, y = verdi, col = BNP_Type))+
labs(title = "Prosentvis endring i BNP (nominelt og reelt)") +
theme_bw()
