...
df_wide <- df_wide %>% 
  mutate(total_indeks = `Konsumprisindeks (vekter)`*`Konsumprisindeks (2015=100)`) 

df_sjekk2 <- df_wide %>% 
  group_by(dato) %>% 
  summarise(sjekk2 = sum(total_indeks, na.rm = TRUE))

 max(abs(100-df_sjekk2$sjekk2))

```

*\[Legger sammen verdiene for totalindeks for å se om det ligger i nærheten av basisåret. Den verdien som ligger lengst unna har en differanse på 24.6\]*
...
*Når kraftprisene øker påvirker dette også næringslivet. Det vil si at produksjonskostnadene øker for bedrifter. Når produksjonskostnadene øker fører dette til at matvarer som produseres bli dyrere. Dette vil i sin tur føre til en konsumprisvekst.\]*
